...
class surat_peminjamans extends CI_Model {
    ...
    /**
     *  Get All data surat_peminjamans
     *
     *  @param limit  : Integer
    ...
    public function get_all($limit, $offset) {
        $this->db->order_by("tgl_peminjaman", "asc");
        $result = $this->db->get('surat_peminjaman', $limit, $offset);
    ...
    public function count() {
        $this->db->from('rinci_surat_peminjaman');
        return $this->db->count_all_results();
    }
    ...
    public function get_search_barang($limit, $offset) {
        ...
        $this->db->like('kode_surat_peminjaman', $keyword);
        ...
        $this->db->limit($limit, $offset);
        $result = $this->db->get("surat_peminjaman");
    ...
    public function save() {
        $data = array(
            'kode_surat_peminjaman' => $this->input->post('no_faktur', TRUE),
    ...
    public function update($id) {
        ...
        $this->db->where('id_penjualan', $id);
        $this->db->update('surat_peminjamans', $data);
    }

    public function lihat() {
        $id = $this->input->get_post('id');
        $query = $this->db->query("select * from surat_peminjaman tp
                            inner join rinci_surat_peminjaman trp on tp.kode_surat_peminjaman = trp.no_surat_peminjaman 
                            where tp.kode_surat_peminjaman='$id'");
        return $query;
    }
    ...
    public function destroy($id) {
        ...
        $this->db->where('kode_surat_peminjaman', $id);
        ...
        /*
          $this->db->where('kode_surat_peminjaman', $id);
          $this->db->update('surat_peminjaman', array("hapus" => "1"));



          $this->db->where("no_surat_peminjaman", $id);
          $this->db->update("rinci_surat_peminjaman", array("hapus" => "1"));
         * 
         */
    }
    ...
    public function no_urut() {
        ...
        $today = date('Ymd');
        $query = $this->db->query("select max(kode_surat_peminjaman) as last from surat_peminjaman where kode_surat_peminjaman like '%$today%'");
    ...
    public function tambahBarang() {
        $data = array(
            'no_surat_peminjaman' => $this->input->get_post("no_faktur"),
        ...
        $this->db->insert("rinci_surat_peminjaman", $data);
    }
    ...
    public function cetakFaktur($no_faktur) {
        ...
        $query = $this->db->query("select * from surat_peminjaman tp
                            inner join rinci_surat_peminjaman trp on tp.kode_surat_peminjaman = trp.no_surat_peminjaman 
                            where tp.kode_surat_peminjaman='$no_faktur'");
        return $query;
    }
    ...
    public function cetakRetur($no_peminjaman) {
        //$id = $this->input->get_post("no_faktur");
        //var_dump($no_faktur);
        $query = $this->db->query("SELECT * FROM surat_retur sr
                            INNER JOIN rinci_surat_retur rsr ON sr.no_surat_peminjaman = rsr.no_surat_peminjaman 
                            WHERE sr.no_surat_peminjaman='$no_peminjaman'");
        return $query;
    }
    ...
    public function cetakInvoice($no_peminjaman) {
        //$id = $this->input->get_post("no_faktur");
        //var_dump($no_faktur);
        $query = $this->db->query("SELECT * FROM tabel_penjualan sr
